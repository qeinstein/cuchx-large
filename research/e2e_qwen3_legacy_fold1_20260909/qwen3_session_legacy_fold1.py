...
SYSTEM = (
    "You solve privacy-preserving video multiple-choice questions. Multiple videos belong "
    "to one recording session and are shown in recording order. Solve every question jointly. "
    "Sibling videos share an action pool and one latent action order; their emotion labels "
    "follow a one-to-one slow/neutral/fast manner protocol. For single, emotion, combination "
    "and object questions return one letter; for multi return all correct letters sorted; for "
    "sequence return all four letters in temporal order. Return exactly one KEY=ANSWER line "
    "for every listed question, preserving the listed keys, with no explanation."
)
...
def target_for(clips: list[tuple[str, pd.DataFrame]]) -> str:
    return "\n".join(f"{key}={row.answer}" for key, row, _ in keys_for(clips))
...
def parse_answers(text: str, clips) -> dict[str, str]:
    ...
    found = dict(re.findall(r"(V\d+Q\d+)\s*=\s*([A-D]+)", text.upper()))
    return {key: found[key] for key in expected_keys if key in found}
...
adapter_dir = OUT / f"qwen3_session_fold{FOLD}_adapter"
