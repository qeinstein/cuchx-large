"""Build submission_v4: Fusing exact sequence guarantees, high-margin specialists, and +14.1% IMU emotion ensemble."""
...
def main():
    print("Loading sensor and visual feature caches...")
    ...
    # 5. Train 40-action Multi-Label Multimodal Detector for Multi
    ...
    # 6. Train +14.1% CV Calibrated IMU Emotion Ensemble (RF-k60 + Logistic Regression)
    tr_emo = train_df[train_df.category == "emotion"].copy()
    tr_emo["target"] = tr_emo.apply(lambda r: str(r[r.answer]).strip().lower(), axis=1)

    X_tr_emo = np.array([X_sens_tr[path_to_idx[p]][:120] for p in tr_emo.path])
    scaler_emo = StandardScaler()
    X_tr_emo_s = scaler_emo.fit_transform(np.nan_to_num(X_tr_emo))
    X_te_emo_s = scaler_emo.transform(np.nan_to_num(X_sens_te[:, :120]))

    sel_emo = SelectKBest(f_classif, k=60)
    X_tr_emo_sel = sel_emo.fit_transform(X_tr_emo_s, tr_emo.target)
    X_te_emo_sel = sel_emo.transform(X_te_emo_s)

    rf_emo = RandomForestClassifier(n_estimators=500, random_state=42, n_jobs=-1, class_weight="balanced")
    rf_emo.fit(X_tr_emo_sel, tr_emo.target)
    classes_rf_emo = list(rf_emo.classes_)

    lr_emo = LogisticRegression(C=0.1, max_iter=1000, class_weight="balanced")
    lr_emo.fit(X_tr_emo_s, tr_emo.target)
    classes_lr_emo = list(lr_emo.classes_)

    for idx, row in test_df[test_df.category == "emotion"].iterrows():
        p = row["path"]
        if p in test_path_to_idx:
            c_idx = test_path_to_idx[p]
            p_rf = rf_emo.predict_proba(X_te_emo_sel[c_idx : c_idx + 1])[0]
            p_lr = lr_emo.predict_proba(X_te_emo_s[c_idx : c_idx + 1])[0]
            scores = []
            for opt in ["A", "B", "C", "D"]:
                val = str(row[opt]).strip().lower()
                score_rf = p_rf[classes_rf_emo.index(val)] if val in classes_rf_emo else 0.0
                score_lr = p_lr[classes_lr_emo.index(val)] if val in classes_lr_emo else 0.0
                scores.append(0.6 * score_rf + 0.4 * score_lr)
            best_opt = ["A", "B", "C", "D"][int(np.argmax(scores))]
            qid = row["qa_id"]
            curr_pred = pred_map[qid]
            if best_opt != curr_pred:
                changes.append((qid, "IMU_emotion_ensemble_override", curr_pred, best_opt))
                pred_map[qid] = best_opt

    print(f"Total deliberate changes across test set: {len(changes)}")
    category_counts = {}
    for ch in changes:
        category_counts[ch[1]] = category_counts.get(ch[1], 0) + 1
    for cat, count in category_counts.items():
        print(f"  {cat}: {count} questions updated")
    ...
    out_file = ROOT / "submission_v4.csv"
